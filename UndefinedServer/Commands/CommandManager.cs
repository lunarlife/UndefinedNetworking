...
namespace UndefinedServer.Commands
{
    public class CommandManager
    {
        ...
        public static Command GetCommnand(string name)
        {
            if (!ECommands.Contains(name)) throw new CommandException($"unknown command {name}");
            return ECommands[name];
        }

        public static bool TryGetCommand(string name, out Command? command)
        {
            if (ECommands.Contains(name))
            {
                command = ECommands[name];
                return true;
            }
            command = null;
            return false;
        ...
        public static T RegisterCommand<T>() where T : Command, new()
        {
            ...
            for (var i = command.Parameters.Length - 1; i >= 0; i--)
            {
                ...
                if(current is IRepeatable && i != command.Parameters.Length - 1) 
                    throw new CommandException(
                        $"Register command {nameof(T)} failed. Only latest argument can be {nameof(IRepeatable)}");
        ...
        private static bool IsComplete(ServerPlayer sender, string[] pr, int commandId, out ParameterType? par)
        {
        ...
        private static void IsComplete(ServerPlayer sender, CommandCompleteRequest request)
        {
            if (ECommands.Contains(request.CommandId))
            {
                ...
                if (command.Parameters is { } parameters)
                {
                    ...
                    {
                        var parameter = command.Parameters[i];
                        completes[i] = new ParameterCompletesPacket(parameter.GetCompletes(sender), parameter is IRepeatable, parameter.Title);
                    }
        ...
        private void OnCommandPacket(PacketReceiveEvent e)
        {
            ...
            {
                IsComplete(UServer.CurrentGame.GetPlayer(e.Client.Identifier), ccp);
                return;
            }
            ...
            if (command.Parameters is { } parameters)
            {
                if(packet.Parameters is null || (command.Parameters[^1] is not IRepeatable && packet.Parameters?.Length != command.Parameters.Length)) 
                {   
                ...
                for (var i = 0; i < packet.Parameters!.Length; i++)
                {
                    ...
                    prs.Add(new CommandParameter(packet.Parameters[i]));
                }
            }
